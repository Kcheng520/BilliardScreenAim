 ...
 *
 * 颜色阈值有两个来源,优先级从高到低:
 *  1. 用户在 CalibrationActivity 里手动标定过的值(存在 CalibrationStore 里)
 *  2. 下面写死的默认值(仅作为兜底示例,不同游戏效果差异很大)
 */
class BilliardDetector(context: Context) {

    // ---------- 默认值(仅在用户没有手动标定时使用) ----------
    private val TABLE_HSV_DEFAULT_LOWER = Scalar(70.0, 70.0, 20.0)
    private val TABLE_HSV_DEFAULT_UPPER = Scalar(100.0, 255.0, 160.0)

    // 母球默认颜色范围:高亮度、低饱和度(等价于原来的 BALL_SAT_MAX=80 / BALL_VAL_MIN=180)
    private val BALL_HSV_DEFAULT_LOWER = Scalar(0.0, 0.0, 180.0)
    private val BALL_HSV_DEFAULT_UPPER = Scalar(180.0, 80.0, 255.0)

    private val GUIDE_HSV_DEFAULT_LOWER = Scalar(0.0, 0.0, 120.0)
    private val GUIDE_HSV_DEFAULT_UPPER = Scalar(180.0, 90.0, 255.0)

    // 实际生效的阈值:优先用标定值
    private val tableRange = CalibrationStore.loadRange(
        context, CalibrationStore.CATEGORY_TABLE, TABLE_HSV_DEFAULT_LOWER, TABLE_HSV_DEFAULT_UPPER
    )
    private val ballRange = CalibrationStore.loadRange(
        context, CalibrationStore.CATEGORY_BALL, BALL_HSV_DEFAULT_LOWER, BALL_HSV_DEFAULT_UPPER
    )
    private val guideRange = CalibrationStore.loadRange(
        context, CalibrationStore.CATEGORY_GUIDE, GUIDE_HSV_DEFAULT_LOWER, GUIDE_HSV_DEFAULT_UPPER
    )
    ...
    private fun detectTable(hsv: Mat): List<Point>? {
        val mask = Mat()
        Core.inRange(hsv, tableRange.first, tableRange.second, mask)
    ...
    private fun detectCueBall(bgr: Mat, hsv: Mat, tableCorners: List<Point>?): Pair<Point, Double>? {
        val mask = Mat()
        Core.inRange(hsv, ballRange.first, ballRange.second, mask)
    ...
    private fun detectGuidelineByColor(hsv: Mat, tableCorners: List<Point>?): Pair<Point, Point>? {
        val mask = Mat()
        Core.inRange(hsv, guideRange.first, guideRange.second, mask)
